...
export function ServiceCard({ service, index = 0 }: ServiceCardProps) {
  ...
  const prev = (e: React.MouseEvent) => {
    e.stopPropagation();
    setImgIndex((i) => (i === 0 ? service.images.length - 1 : i - 1));
  };
  const next = (e: React.MouseEvent) => {
    e.stopPropagation();
    setImgIndex((i) => (i === service.images.length - 1 ? 0 : i + 1));
  };

  const contactUrl = `/contact?service=${encodeURIComponent(service.name)}`;

  const categoryLabel =
    service.categoryId === "residential"
      ? "Residential"
      : service.categoryId === "commercial"
      ? "Commercial"
  ...
  return (
    ...
    >
      {/* ── Image ──────────────────────────────────────────────── */}
      <div className={`relative aspect-square overflow-hidden ${service.imageFit === "contain" ? "bg-white p-4" : "bg-ink-100"}`}>
        <AnimatePresence mode="wait">
          ...
          >
            <Image
              ...
              alt={`${service.name} – Rajendra Glass House`}
              fill
              sizes="(max-width: 640px) 100vw, (max-width: 1280px) 50vw, 33vw"
              className={`transition-transform duration-700 ease-out group-hover:scale-[1.04] ${service.imageFit === "contain" ? "object-contain p-4" : "object-cover"}`}
              priority={index < 4}
            />
          </motion.div>
        </AnimatePresence>
          ...
          {service.specs.map((spec, i) => (
            <div
              ...
              className={`flex items-baseline justify-between py-2 text-[12px] ${
                i < service.specs.length - 1 ? "border-b border-ink-100" : ""
              }`}
            >
